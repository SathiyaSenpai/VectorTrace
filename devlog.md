...
- **Prefix-Based Text Matching**: Added an 80%-prefix overlap check to `isTextMatch` for long strings, fixing false-positive `TEXT_CONTENT_CHANGED` when numbers are inserted near the end of paragraph text (e.g., `"...permission."` → `"...permission 123."`).
- **Two-Pass Page Search**: Enhanced `findElementOnPage` with a fuzzy second pass using `isTextMatch` after the exact-match pass, so selector-drifted elements with minor text edits are still found on the page and returned as OK.
- **HEALED Status Wiring Fix**: Ensured that the extraction re-run after healing returns `OK` (so the pending heal promotion to `HEALED` fires), by fixing text matching that previously caused the re-extraction to fail for the newly stored content.
- **TAG_CHANGED Full UI Wiring**: Added `TAG_CHANGED` to `HEALABLE_STATUSES` for auto-heal, to `STATUS_DIAGNOSIS` and `problemCount` in `ExtractionResults`, added a 🔀 count pill in the health summary, and rendered a red TAG badge with a "Find Replacement" button.
